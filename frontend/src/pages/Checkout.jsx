...
function Checkout() {
  ...
  const { cartItems, removeFromCart, updateQuantity, clearCart, isCartInitialized } = useCart();
  console.log("cart from chekcout", isCartInitialized)
  ...
  const total = sorted?.reduce((sum, item) => sum + item.price * item.quantity, 0);
          ...
          <>
            ...
            <div className="bg-white rounded-xl shadow-lg p-6 mb-8 border">
              <div className="flex flex-col lg:flex-row justify-between">
                <div>
                  ...
                  <p className="text-lg text-gray-600">{totalItems} item{totalItems > 1 ? 's' : ''} in your cart</p>
                </div>
                <div className="text-right">
                  <p className="text-sm text-gray-600">Total</p>
                  <p className="text-4xl font-bold text-blue-600 mb-4">${total.toFixed(2)}</p>
                  <div className="flex gap-3 justify-end">
                    <button
                      onClick={clearCart}
                      className="bg-gray-100 hover:bg-gray-200 text-gray-700 px-6 py-3 rounded-xl font-semibold border border-gray-200"
                    >
                      Clear Cart
                    </button>
                  </div>
            ...
            {sorted.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {sorted.map(item => (
                  <div key={item.id} className="bg-white rounded-2xl shadow border overflow-hidden">
                    <ProductItem
                      product={item}
                      showAddToCart={false}
                    ...
                    <div className="p-4 border-t bg-gray-50">
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="px-3 py-1 rounded border">{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.id, item.quantity + 1)}
              ...
              <div className="text-center py-16 bg-white rounded-2xl shadow border">
                <p className="text-gray-500 text-xl">No items match your filters or search.</p>
                <button
                  onClick={() => {
                    setSearchTerm('');
                    setSortBy('');
